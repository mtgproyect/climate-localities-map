(() => {
  ...
  const ICON_SVGS = {
    ...
    "clear-night": `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M19.5 14.5A8.5 8.5 0 1 1 9.5 4.5a7 7 0 0 0 10 10Z"></path>
      </svg>`,
    partly: `
      <svg viewBox="0 0 24 24" aria-hidden="true">
  ...
  function createWeatherIcon(locality, record, kind = "locality") {
    ...
    const width = kind === "station" ? 78 : 72;
    const height = kind === "station" ? 38 : 34;
    const classNames = [
      "weather-chip",
      `marker-${visualCategory}`,
    ...
    const icon = L.divIcon({
      ...
      html: `
        <div class="${classNames}">
          <span class="weather-chip__glyph">${ICON_SVGS[visualCategory] || ICON_SVGS.unknown}</span>
          <span class="weather-chip__temp">${tempText}</span>
        </div>
      `,
    ...
    return `
      <div class="popup-card">
        <div class="popup-title">
          ...
          <span class="popup-condition-icon">${WEATHER_SYMBOLS[visualCategory] || WEATHER_SYMBOLS[category]}</span>
        </div>
  ...
  function createLocalityMarkers() {
    ...
    const markers = state.localities.map((locality) => {
      ...
      const marker = L.marker([locality.lat, locality.lon], {
        icon: createWeatherIcon(locality, record, "locality"),
        title: locality.name,
        riseOnHover: true,
      });
  ...
  function createStationMarkers() {
    ...
    for (const locality of state.localities) {
      ...
      const marker = L.marker([lat, lon], {
        pane: "stationPane",
        icon: createWeatherIcon(stationLocality, record, "station"),
  ...
  function updateAllMarkerIcons() {
    ...
    for (const marker of state.localityMarkers) {
      ...
      marker.setIcon(createWeatherIcon(marker.locality, record, "locality"));
    }

    for (const marker of state.stationMarkers) {
      const record = state.observations.get(String(marker.stationNumber));
      marker.setIcon(createWeatherIcon(marker.locality, record, "station"));
    }
  }
  ...
  function renderSearchResults(query) {
    ...
    for (const locality of matches) {
      ...
      button.type = "button";
      button.innerHTML = `
        <span class="search-result-icon">${WEATHER_SYMBOLS[visualCategory] || WEATHER_SYMBOLS.unknown}</span>
        <span class="search-result-copy">
